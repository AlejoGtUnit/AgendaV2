...
{
    var jqxhr = $.get(urlService, { filtro:"portada" })
    .done(function(data)
    {
        if (data)
        {
            if (data.eventos.length > 0)
            {
                ...
                var eventosFechaMasProxima = [];
                for (var x = 0; x < data.eventos.length; x++)
                {
                    if (eventosFechaMasProxima.find(x => { return x.urlEvento == data.eventos[x].urlEvento; } == undefined)
                    {
                         eventosFechaMasProxima.push(data.eventos[x])
                    }
                }
                data.eventos = eventosFechaMasProxima;
